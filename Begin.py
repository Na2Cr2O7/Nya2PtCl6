...
# service=Service()
# service.executable_path=os.path.join(os.getcwd(),'chromedriver.exe')
# options=Options()
# options.binary_location
# options.binary_location = r"D:\portapps\ungoogled-chromium-portable\ungoogled-chromium-portable.exe"
...
def getTrainingDataReply(question,answers):
    ...
    try:
        for answer in answers:
            ...
            json.dump(data,fp=open('trainingData2.jsonl','a',encoding='utf8'))
    except Exception as e:
        print(e)
        Status.setStatus("getTrainingDataReply() Exception:"+str(e))
...
def getTrainingDataPost(question):
    ...
    try:
        ...
        for i in a:
            answer=i.find_element(By.TAG_NAME,'p').text
            data={"question":question,"answers":answer}
            json.dump(data,fp=open('trainingData2.jsonl','a',encoding='utf8'))
    except Exception as e:
        print(e)
        Status.setStatus("getTrainingDataPost() Exception:"+str(e))
...
TESTINT=-12
SLPTIME:int=300
SLPTIMEFORREPLY:int=300

MONITORMODE=False
if BatteryStatus.restTime():
        print('休眠中,打开监视模式.')
        feed()
        MONITORMODE=True
...
def getAnswerUsingOllama(question):
    Status.setStatus(f"getAnswerUsingOllama({question[:30]})...")
    answer=getAnswerbyOllama(question)
    Status.setStatus(f"getAnswerUsingOllama({question[:30]})->{answer[:30]}")
    return  answer
def getAnswerLite(question):
    return getAnswerUsingOllama(question)

def getAnswerforNovel(question,**kwargs):
    Status.setStatus(f"getAnswerforNovel({question[:30]})...")
    feed()
    getAnswerIn2(NOVALPKL,question)
    
    text=getAnswerIn2(NOVALPKL,question).replace(';','\n')
...
def getAnswer(question,**kwargs)->str:
    ...
    questioninString=question+'#'.join(prompts)
    imageDir=kwargs.get('imageDir',None)

    Status.setStatus(f"getAnswer({question[:30]})...")
    print(questioninString)

    feed()
    text=getAnswerIn2(questioninString,imageDir)
...
def getPost():
    ...
    rQ.content=getAnswerIn2(POSTPKL,rQ.title)
...
if login:
    ...
    if Cookie.cookieExists():
        try:
            ...
            Sleep(12)
            Cookie.loadCookie(wd)
            Sleep(12)
            Status.setStatus("Cookie loaded")
...
while True:
    ...
    if fpr==0:
            ...
            try:
                ...
                for i in repld:
                    ...
                    if name+content not in replied and name not in Blacklist_:
                        ...
                        writereplied()
                        question=content+originalcontent+'\n#原帖:'+originalPost
                        ...
                        if image:
                            ...
                            try:
                                answer=getAnswer(content+originalcontent+'\n#原帖:'+originalPost)
                            except Exception as e:
                                answer='喵。'
                        answer=answer[:500]
                        if not getTrainingDataReplyFlag:
                            getTrainingDataReply(content,answersToTrain)
                ...
                try:
                    ...
                    for i in repld:
                        ...
                        try:
                            ...
                            print('''name=i.find_element(By.CLASS_NAME,'mhy-account-title__name').text Failed''')
        ...
        try:
            ...
            Status.setStatus(f"Failed to fetch images:{e}")
            fpr+=1
            continue
        try:
            _title=imageLs[0]
                ...
                try:
                    ...
                    if title not in replied and author not in list(blacklist.getUsersWithCountOverTen()):
                        ...
                        try:
                            ...
                            if image:
                                imageDescription=VisionUtil.getImageDescription(image)
                                question=title+','+content+',图片描述:'+imageDescription
                                answer=getAnswer(title+','+content)
                            else:
                                answer=getAnswer(title+','+content)
                            answer=answer[:1000]
                            getTrainingDataPost(question)
                            inputBox=wd.find_element(By.ID,'reply').find_element(By.CLASS_NAME,"ql-editor")
                            try:
