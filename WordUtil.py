...
def replaceAfterString(text, replacement):
    pattern = r'我是(.*?)[，。]|我是(.*?)$'
    result = re.sub(pattern, lambda m: '我是' + replacement+'。', text)
    return result
def replaceAfterList(text, replacements):
    for replacement in replacements:
        text = replaceAfterString(text, replacement)
    return text
def replaceAfter(text, replacements):
    if isinstance(replacements, str):
        text = replaceAfterString(text, replacements)
    elif isinstance(replacements, list):
        text=replaceAfterList(text, replacements)
    return text