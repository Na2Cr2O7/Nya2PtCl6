...
from Mm2Vmodel_minimind import MiniMindConfig, MiniMindForCausalLM
from transformers import AutoTokenizer, AutoModelForCausalLM, TextStreamer
from Mm2Vmodel_vlm import MiniMindVLM, VLMConfig
from Mm2Vmodel_lora import *
...
tokenizer = AutoTokenizer.from_pretrained('./VisionModel')
ckp='./VisionModel/MiniMind2V.pth'
model = MiniMindVLM(lm_config, vision_model_path="./VisionModel/vision_model/clip-vit-base-patch16")
...
@app.route('/<imageDir>', methods=['POST'])
def get_answer( imageDir=None):
    if imageDir is None:
        return flask.jsonify({'answer': 'No image directory provided'})
    viewImage(os.path.abspath(imageDir))
    print('\n\n')
    ...
    with open('replyHistoryVision.txt','a') as f:
        f.write(dt)
